...
export default function FlowChart(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await fetch("https://localhost:7064/api/Dashboard/FlowData")
            if(!res.ok){
    ...
    useEffect(()=> {
        if(chartRef.current){
            ...
            flowData.forEach((datum) => {
                if(datum.timeSeriesId === 61865010){        
            ...
            flowData.forEach((datum) => {
                if(datum.timeSeriesId === 61873010){        
            ...
            flowData.forEach((datum) => {
                if(datum.timeSeriesId === 61896010){        
            ...
            const newChart = new Chart(context, {
                ...
                options : {
                    ...
                    plugins:{
                        title:{
                            display: true,
                            text:"Flow rate at Monitoring Stations on the Almond",
                            position: 'bottom',
                            color: '#084075'
                        }
                    }
